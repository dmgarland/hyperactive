...
module PaginatingFind
  ...
  module ClassMethods
    ...
    def find_with_pagination(*args)
      options = args.extract_options! #extract_options_from_args!(args) 
    ...
    # From http://www.igvita.com/blog/2007/02/20/ferret-pagination-in-rails/
    # Paginating find integration for an acts_as_ferret search.
    def paginating_ferret_search(options)
       count = find_by_contents(options[:q], {}, {:conditions => options[:conditions], :include => options[:include]}).total_hits
       PagingEnumerator.new(options[:page_size], count, false, options[:current], 1) do |page|
          ...
          res = find_by_contents(options[:q], {:offset => offset, :limit => limit}, {:conditions => options[:conditions], :include => options[:include]})
        end
    end
    ...
    def paginating_ferret_multi_search(options)
       count = multi_search(options[:q], [User], {:limit => :all, :lazy => true}).total_hits
    
       PagingEnumerator.new(options[:page_size], count, false, options[:current], 1) do |page|
          offset = (options[:current].to_i - 1) * options[:page_size]
          limit = options[:page_size]
    
          multi_search(options[:q], [User], {:offset => offset, :limit => limit})
        end
    end    
