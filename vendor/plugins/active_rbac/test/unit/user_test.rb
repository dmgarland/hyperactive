...
class UserTest < Test::Unit::TestCase
  ...
  def test_add_with_valid_data_should_work
    ...
    assert_equal 'NewUser', user.login
  ...
  def test_should_block_creation_with_login_with_invalid_characters
    ...
    for char in invalid_chars do
      ...
      assert_equal 1, user.errors.count
      assert_equal "should consist only of letters, numbers, and underscores.", user.errors["login"]
    end
  end
  ...
  def test_login_failures_should_be_increased_on_wrong_login
    assert_equal 0, User.find(users(:icarus_user).id).login_failure_count
    assert_nil User.find_with_credentials('Icarus', 'password123')
    assert_equal 1, User.find(users(:icarus_user).id).login_failure_count
  end
  ...
  def test_should_block_editing_with_invalid_state
    ...
    assert_equal 1, user.errors.count
    assert_equal 'must be a valid new state from the current state.', user.errors["state"]
  end
  ...
  def test_should_set_update_timestamp_on_editing
    user = User.find users(:icarus_user).id

    old_time = user.updated_at
    user.email = "foo@foo.org"

    assert user.save 
   
    assert old_time < user.updated_at
  ...
  def test_should_allow_editing_login_with_valid_value
    user = User.find @icarus_user.id
    user.login = 'NotIcarus'

    assert user.save
    user.reload

    assert_equal 'NotIcarus', user.login
  end
  ...
  def test_return_false_on_has_role_if_correct
    # user with no role
    user = User.find @icarus_user.id
    assert_equal FalseClass, user.has_role?(@greek_heroes_role.title).class
  ...
  def test_return_true_on_has_role_if_correct
    ...
    assert_equal TrueClass, user.has_role?(@greek_heroes_role.title).class
  end

  def test_return_false_on_has_permission_if_correct
    # user with no permission
    user = User.find @icarus_user.id
    assert FalseClass, user.has_permission?(@access_olymp_permission.title).class
  ...
  def test_return_true_on_has_permission_if_correct
    ...
    assert TrueClass, user.has_permission?(@access_olymp_permission.title).class
  end

  def test_is_not_anonymous
    user = User.find @perseus_user.id
