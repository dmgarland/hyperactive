...
class StaticPermissionTest < Test::Unit::TestCase
  ...
  def test_should_ignore_double_assigned_roles
    permission = StaticPermission.find(static_permissions(:slay_monsters_permission).id)
    role = Role.find roles(:greek_heroes_role).id
    
    permission.roles << role
    
    #assert_equal 1, permission.roles.length
    #assert_equal [roles(:greek_heroes_role)].sort {|a,b| a.id <=> b.id}, permission.roles.sort {|a,b| a.id <=> b.id}
  end
