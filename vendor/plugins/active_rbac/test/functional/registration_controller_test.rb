...
require 'active_rbac/registration_controller'

# Re-raise errors caught by the controller.
#class ActiveRbac::RegistrationController; def rescue_action(e) raise e end; end

class ActiveRbac::RegistrationControllerTest < ActionController::TestCase
  ...
  tests ActiveRbac::RegistrationController  
  ...
  def test_should_send_registration_email_on_valid_post_to_register
    ...
    email = @emails.first
    assert_equal ActiveRbac.mailer_subject_confirm_registration, email.subject
    ...
    # TODO: if there was a routes file, this might work.
  ...
  def test_should_send_changed_password_email_on_valid_new_password_request
    ...
    assert_equal 1, @emails.length
    email = @emails.first
    assert_equal ActiveRbac.mailer_subject_lost_password, email.subject
    assert_equal [ 'activerbac@localhost' ].sort {|a,b| a.id <=> b.id}, email.from.sort {|a,b| a.id <=> b.id}
  ...
  def test_should_fail_on_empty_registration_request
    ...
    assert_match /Email must be a valid email address/, @response.body
  end
  ...
  def test_should_fail_on_invalid_registration_request_with_invalid_chars_in_login
    ...
    assert_response :success
    assert_template 'register'
    assert_tag :tag => 'li', :content => 'User name should consist only of letters, numbers, and underscores.'
  end
  ...
  def test_should_fail_on_invalid_registration_request_with_too_short_password
    ...
    assert_response :success
    assert_match /Password must have between 6 and 64 characters/, @response.body
  end
